...
public class ConfigItem implements ObjectSerializer<ConfigItem> {
    ...
    // no need for a page if the itemstack can redirect to another pane
    ...
    public Map<Integer, GuiItem> getGuiItems() {
        ...
        GuiItem guiItem = new GuiItem(itemStack, e -> actions.runFor((Player) e.getWhoClicked()));
        slots.forEach((s) -> items.put(s, guiItem));
        return items;
    }
